...
class Visualizer:
    ...
    def plot_simulation(self, paths: Dict[str, List[Tuple[float, float]]], start: Tuple[float, float], goal: Tuple[float, float]):
        ...
        plt.xlabel("X (km)")
        plt.ylabel("Y (km)")
        self._plot_points(plt.gca(), (start[0] / 1000.0, start[1] / 1000.0), (goal[0] / 1000.0, goal[1] / 1000.0))
        plt.tight_layout()
        ...
        plt.streamplot(X_sub / 1000.0, Y_sub / 1000.0, u_grid, v_grid, density=1.2, color='white', linewidth=0.6, arrowsize=1.0)
        ...
        plt.contourf(self.map.X / 1000.0, self.map.Y / 1000.0, self.map.dem, 50, cmap='gist_earth', alpha=0.6)
        plt.streamplot(X_sub / 1000.0, Y_sub / 1000.0, u_grid, v_grid, density=0.8, color=(0, 0, 1, 0.4), linewidth=0.4)

        has_path = False
        for idx, (label, path) in enumerate(paths.items()):
            if path:
                has_path = True
                p = np.array(path) / 1000.0
                if 'Energy' in label:
                    c, s, lw = 'lime', '-', 3
                else:
                    c, s, lw = 'red', '--', 2
